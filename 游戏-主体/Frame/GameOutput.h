...
public:
	...
	//设置标题
	void SetWindowTitle(const char* n);

	//Alpha混合
	bool SetAlphaBlend(int dx, int dy, int dw, int dh, int sx, int sy, int Alpha, const char* bmp); 

	//Alpha混合带透明(不可用)
	bool tSetAlphaBlend(int dx, int dy, int dw, int dh, int sx, int sy, int Alpha, const char* bmp, unsigned int tc = RGB(0, 255, 0)); 
};
